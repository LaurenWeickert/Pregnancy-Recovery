  ...
  return (
    <div className="flex flex-col w-full min-h-screen pt-20 pb-32 bg-white relative">
      ...
      {showScrollDown && messages.length > 0 && (
        <button
          className="fixed bottom-28 right-6 z-30 bg-white/60 rounded-full p-3 shadow-lg border border-[#1F3D1F]/20 transition-all duration-200 backdrop-blur-sm opacity-50 hover:opacity-90"
          style={{ boxShadow: '0 4px 16px rgba(80,0,120,0.10)' }}
          onClick={scrollToBottom}
          aria-label="Scroll to bottom"
        >
          <ChevronDown className="w-7 h-7 text-[#1F3D1F]" />
        </button>
      )}
          ...
          >
            <div
              className={`max-w-[80%] rounded-lg p-3 shadow-sm transform transition-all duration-200 hover:scale-[1.01] ${
                message.isAI
                  ? 'bg-[#1F3D1F]/10 text-[#1F3D1F]'
                  : 'bg-blue-100 text-blue-900'
              }`}
            >
              {message.isAI ? (
                <ReactMarkdown
                  className="prose prose-sm max-w-none text-[#1F3D1F]"
                  components={{
      ...
      {messages.length === 0 && suggestions.length > 0 ? (
        <div className="fixed inset-0 flex flex-col items-center justify-center z-10 pointer-events-none">
          <div className="bg-white/90 rounded-xl shadow-lg px-10 py-8 border border-[#1F3D1F]/20 flex flex-col items-center max-w-lg w-full pointer-events-auto">
            <h2 className="mb-4 text-xl font-semibold text-[#1F3D1F]">Try asking about...</h2>
              ...
              {suggestions.map((suggestion, idx) => {
                ...
                return (
                  <button
                    ...
                    className="p-3 px-6 bg-[#1F3D1F]/10 hover:bg-[#1F3D1F]/20 rounded-full text-base text-[#1F3D1F] border border-[#1F3D1F]/20 transition-colors duration-200 ease-in-out shadow-md font-medium"
            ...
            {suggestions.map((suggestion, idx) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => handleStarterClick(suggestion)}
                  className="p-2 px-4 bg-[#1F3D1F]/10 hover:bg-[#1F3D1F]/20 rounded-full text-sm text-[#1F3D1F] border border-[#1F3D1F]/20 transition-colors duration-200 ease-in-out shadow-sm"
        ...
        <div className="flex items-center space-x-3 w-full p-4 bg-white">
          <div className="flex items-center space-x-3 w-full max-w-5xl mx-auto bg-white px-6 py-4 rounded-xl shadow-sm">
            <input
              ...
              className="flex-1 rounded-full px-5 py-3 border-2 border-[#1F3D1F]/20 focus:outline-none focus:ring-2 focus:ring-[#1F3D1F]/40 focus:border-[#1F3D1F]/40 transition-all duration-200 shadow-sm text-gray-800 placeholder-gray-500"
              disabled={isLoading}
              autoFocus
            />
            <button
              ...
              className="bg-[#1F3D1F] text-white rounded-full p-3 hover:bg-[#1F3D1F]/80 disabled:opacity-50 transition-colors duration-200 hover:shadow-md"
            >
              {isLoading ? (
